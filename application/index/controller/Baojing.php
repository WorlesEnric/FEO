...
class Baojing extends AuthController
{
   ...
   header('Content-Type: application/json');
	...
	{
		$result = Db::table('smstotal')->field('id,probe,tUp,tDown,hUp,hDown,telephone1,telephone2,status,alarmSmsNumLimit,dayNumLimit,delayTime,spaceTime')->select();
		$response = [
			'rows' => $result
	...
	public function shezhi()
	{
		...
		Db::table('smstotal')->where('id', $id)->update([
			'probe' => $probe,
			'tUp' => $tUp,
			'tDown' => $tDown,
			'hUp' => $hUp,
	...
	public function smsSwitch()
	{
		...
		Db::name('smstotal')->where('id', $id)->update(['status' => $smsSwitch]);

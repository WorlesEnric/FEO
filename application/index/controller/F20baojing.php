...
class F20baojing extends AuthController
{
	...
	public function add()
	{
		...
		Db::table('f20parameter1')->insert($data);
   ...
   header('Content-Type: application/json');
	...
	{
		$result = Db::table('f20parameter1')->field('id,probe,tUp,tDown,hUp,hDown,telephone1,telephone2,status,alarmSmsNumLimit,dayNumLimit,delayTime,spaceTime')->select();
		$response = [
			'rows' => $result
	...
	public function shezhi()
	{
		...
		Db::table('f20parameter1')->where('id', $id)->update([
			'probe' => $probe,
			'tUp' => $tUp,
			'tDown' => $tDown,
			'hUp' => $hUp,
	...
	public function smsSwitch()
	{
		...
		Db::name('f20parameter1')->where('id', $id)->update(['status' => $smsSwitch]);

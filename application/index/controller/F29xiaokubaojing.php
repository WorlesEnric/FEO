...
class F29xiaokubaojing extends AuthController
{
	...
	public function add()
	{
		...
		Db::table('f29smaparameter')->insert($data);
   ...
   header('Content-Type: application/json');
	...
	public function f29xiaokubaojing()
	{
	...
	{
		$result = Db::table('f29smaparameter')->field('id,probe,tUp,tDown,hUp,hDown,telephone1,telephone2,status,alarmSmsNumLimit,dayNumLimit,delayTime,spaceTime')->select();
		$response = [
			'rows' => $result
	...
	public function shezhi()
	{
		...
		Db::table('f29smaparameter')->where('id', $id)->update([
			'probe' => $probe,
			'tUp' => $tUp,
			'tDown' => $tDown,
			'hUp' => $hUp,
	...
	public function smsSwitch()
	{
		...
		Db::name('f29smaparameter')->where('id', $id)->update(['status' => $smsSwitch]);
